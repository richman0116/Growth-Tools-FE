        ...
        <>
            ...
            <section className="flex flex-col gap-6 p-4 md:p-8">
                <div className="flex gap-4 items-center">
                    <FilterPopoverTool categories={categories} onSubmitFilter={onFilter} />
                ...
                <div className="grid grid-col-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {
                        !!tools?.length && !isLoading && tools.map((tool) => (
                            <ToolCardInfo key={`tool-card-${tool.id}`} variant={variant} tool={tool} isLoading={false} />
                        ))
                    }
                </div>
                <div className="flex justify-center">
                    {pagination?.hasNextPage && <Button variant="outline" onClick={onNextPage}>View More</Button>}
                </div>
            </section>
        </>
    );
